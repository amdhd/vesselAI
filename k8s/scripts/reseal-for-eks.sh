...
trap 'rm -f /tmp/sealed-full.yaml /tmp/sealed-grafana.yaml' EXIT
...
# ---------------------------------------------------------------------------
# GRAFANA, sealed the same way and for the same reason.
#
# k8s/monitoring/02-grafana-sealedsecret.yaml is also encrypted to the k3d key,
# so on a fresh cluster grafana-secrets never materialises and the pod sits in
# CreateContainerConfigError — GF_SECURITY_ADMIN_PASSWORD references a Secret
# that does not exist. Missed on the first pass because the app's secrets were
# the obvious ones; every SealedSecret in the repo needs re-sealing, not just
# the ones in the app namespace.
#
# The password is generated, not carried over: nothing outside this cluster
# needs to know it, and reusing the laptop's Grafana password would be the same
# mistake as reusing its database password.
# ---------------------------------------------------------------------------
GRAFANA_OUT="k8s/overlays/prod-monitoring/grafana-sealedsecret-patch.yaml"
...
| kubeseal \
    ...
    --format yaml \
    --scope strict \
> /tmp/sealed-grafana.yaml

{
  echo "# GENERATED by k8s/scripts/reseal-for-eks.sh — do not hand-edit."
  echo "# Grafana admin password, sealed against the EKS cluster's key."
  echo "# Retrieve it after deploy with:"
  echo "#   kubectl -n monitoring get secret grafana-secrets \\"
  echo "#     -o jsonpath='{.data.admin-password}' | base64 -d"
  sed '/creationTimestamp: null/d' /tmp/sealed-grafana.yaml
} > "$GRAFANA_OUT"

echo "==> wrote $GRAFANA_OUT (fresh generated admin password)"
